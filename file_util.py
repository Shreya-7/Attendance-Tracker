...
class GoogleFormFile(UploadedFile):
    ...
    def get_date(self):
        ...
        if len(set(dates)) != 1:
            return (
                "There are different dates in this file! Please upload a file with only one date.",
                False,
            )
        ...
        with open(file_path, mode="r") as csv_file:
            ...
            for row in csv_reader:
                if line_count == 0:  # heading row
                    ...
                    check_result = self.heading_check(self.gform_headings, row)
                    if check_result is not True:
                        return check_result
        ...
        with open(file_path, mode="r") as csv_file:
            ...
            for row in csv_reader:
                if line_count == 0:  # heading row
                    ...
                    check_result = self.heading_check(self.teams_headings, row)
                    if check_result is not True:
                        return check_result
...
class Report:
    ...
    def create_filename(self, fields):
        ...
        self.filename = (
            f"{self.db_obj.course_id}_{self.db_obj.batch}_asOf_{max(fields[3:])}"
        )
        if self.type == 0:
    ...
    def make_report(self) -> tuple:
        ...
        for key, value in course["students"].items():
            ...
            if value == {}:
                return (
                    "This course has no/missing attendance details. Please track a date to download a report",
                    False,
                )
